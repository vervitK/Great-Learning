...
from youtube where category_id between 9 and (select category_id from youtube order by category_id desc limit 1) ;

(select category_id from youtube order by category_id desc limit 1) ;
...
select channel_title, video_id,views from youtube order by views desc limit 10 ;
...
select p.customernumber,amount,status from payments p join(select status,customernumber from orders where status like'%cancelled%')o
on p.customernumber=o.customernumber where amount>100000 group by customernumber;
...
select productname , productline, productvendor from products where productcode= Any
(select productcode from orderdetails where ordernumber= any
...
select concat(contactfirstname,'',contactlastname) name ,addressline1, coalesce(addressline1,city,state) , state,country, phone 
from customers where customernumber = any
