...
Select bidder_id,(sum((case when bid_status = 'Won' then 1 else 0 end))/count(*))*100 as 'Percent of Win'
...
Select s.Stadium_id,Stadium_Name,City as Stadium_City,Count(Match_id) as No_of_Matches
...
Select Match_id,if(Toss_winner = Match_Winner,1,0) from Ipl_match;


#Q4.	Show the total bids along with bid team and team name.

Select Bid_Team,Team_Name,sum(No_of_Bids) as Total_no_of_bids 
from ipl_bidding_details ibd
join ipl_team it
...
SELECT * FROM ipl.ipl_team_standings;
Select * from ipl_team;

Select it.Team_id,Team_name,sum(Matches_played),Sum(Matches_won),Sum(Matches_lost) from ipl_team_standings its
join ipl_team it
...
Select player_id, Player_name from ipl_player where player_id in 
(Select player_id from ipl_team_players itp where player_role = 'Bowler' and team_id = 5);

Select ip.Player_id,Player_name,player_role from ipl_player ip
...
Select itp.team_id,Team_name,player_role,count(Player_role = 'All-Rounder') as All_rounder_team_wise from ipl_player ip
join ipl_team_players itp
on itp.player_id = ip.Player_id
join ipl_team it
...
group by team_id
having count(Player_role = 'All-Rounder') > 4
order by All_rounder_team_wise desc
;
